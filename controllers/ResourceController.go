...
func (c *ResourceController) TreeGrid() {
	...
	userid := c.curUser.Id
	//获取用户有权管理的菜单列表（包括区域）
	tree := models.ResourceTreeGridByUserId(userid, 2)
	subTree := make([]*models.Resource, 0)
	for _,  item := range tree {
		if item.Name == "权限管理" ||
		   strings.Index(item.UrlFor, "Resource") >= 0 ||
		   strings.Index(item.UrlFor, "Role") >= 0 ||
		   strings.Index(item.UrlFor, "BackendUser") >= 0 {
			continue
		}
		subTree = append(subTree, item)
	}
	//转换UrlFor 2 LinkUrl
	c.UrlFor2Link(subTree)
	c.jsonResult(enums.JRCodeSucc, "", subTree)
}
