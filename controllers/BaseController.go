...
func (c *BaseController) setBackendUser2Session(userId int) error {
	...
	if !c.curUser.IsSuper {
		usrData := models.BackendUserRecursiveById(userId)
		for _, item := range usrData {
			m.UIds = append(m.UIds, item.Id)
		}
	}
	// 分配坐席
	if agent, err := models.AgentGatewayOneByUserId(userId); err == nil {
		m.ExtNo = strings.TrimSpace(agent.ExtNo)
	}
